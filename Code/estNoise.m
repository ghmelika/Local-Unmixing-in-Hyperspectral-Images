...
y = varargin{1};
if ~isnumeric(y), error('the data set must be an L x N matrix'); end
...
[L N] = size(y); % y is the data set (L x N matrix)
...
Rw=diag(diag(w*w'/N)); 
% if verbose, fprintf(1,'\ncomputing noise correlation matrix\n');end
% Rw=corrcoef(w); % normalized :| is it same as normalized covariance? 
return
